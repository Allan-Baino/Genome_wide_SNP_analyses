...
**The aim of this analysis layout is to detail the prcoess of generating a high confidence dataset of variants and population genetics analyses for African giant pouched rat samples sequenced by the ddRAD method**.
...
**Script 09** lcaVC.sh - This script calls variants (SNPs) using BCFtools https://samtools.github.io/bcftools/bcftools.html, there are multiple variant calling software, algorithmic bias can play a role in exact number of SNPs called however, the biological conclusions from downstream analyses will very likely be similar. After generating the .vcf file a rigorous filtering process ensued to get a high confidence dataset...main aim...acquire a catalog of variants across samples (variants from comparable regions in the genome across all individuals). Nano run on a MiSeq system to validate DNA library yielded 74,145 variant loci, .fq files can be accessed on NCBI BioProject:PRJNA1419331. A larger sequencing effort (NovaSeq run) yielded 626,004 variant loci across 108 individuals. NovaSeq variant loci were filtered using vcftools https://vcftools.sourceforge.net/man_latest.html. A lenient filtering regimen resulted in 2,762 variant loci, this approach was weighed against the objectives of the research (SNP discovery). Technical replicates in the data were used to estimate genotyping error (23.34%) and then removed leaving 2,762 variant loci from 93 individuals. Detailed descriptions on the filtering process can be found in the SNP_fil80.txt file.

**Script 10** popGen_SNPs.R - This R script details select population genetics analyses performed to answer my research objectives. Examples of questions i was interested in answering include; the number of distinct genetic units/popns within my sample size? how much genetic relatedness exists at the popn and indv. level? how genetically diverse were popns? how much genetic differentiation is there among popns? and within the captive popn? i was also interested in profiling the genotpes present among two sets of indvs. grouped by olfactory performance in the captive popn. Data to run this script is available on request.
