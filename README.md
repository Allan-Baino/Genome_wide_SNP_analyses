...
**Script 09** lcaVC.sh - This script calls variants (SNPs) using BCFtools https://samtools.github.io/bcftools/bcftools.html, there are multiple variant calling software, algorithmic bias can play a role in exact number of SNPs called however, the biological conclusions from downstream analyses will very likely be similar. After generating the .vcf file a rigorous filtering process ensued to get a high confidence dataset...main aim...acquire a catalog of variants across samples (variants from comparable regions in the genome across all individuals). Nano run on a MiSeq system (DNA library validation) yielded 74,145 variant loci, larger sequencing effort (NovaSeq run) yielded 626,004 variant loci across 108 individuals. NovaSeq variant loci were filtered using vcftools https://vcftools.sourceforge.net/man_latest.html. A lenient filtering regimen resulted in 2,762 variant loci, this approach was weighed against the objectives of the research (SNP discovery). Technical replicates in the data were used to estimate genotyping error (23.34%) and then removed leaving 2,762 variant loci from 93 individuals. Detailed descriptions on the filtering process can be found in SNP_fil80.txt file.
